...
const navigation = [
  ...
  { name: 'Workflows', href: '/dashboard/workflows', adminOnly: false },
  { name: 'Activity', href: '/dashboard/activity', adminOnly: false },
  { name: 'Credentials', href: '/dashboard/credentials', adminOnly: false },
  { name: 'Clients', href: '/dashboard/clients', adminOnly: true },
];

export function Navbar() {
  const pathname = usePathname();
  const { isPlatformAdmin } = useClient();
  ...
  return (
    <nav className="sticky top-0 z-40 bg-background-100 shadow-[inset_0_-1px] shadow-gray-alpha-400">
      <div className="flex h-[46px] items-center px-2 md:px-4 relative" ref={navRef}>
        {navigation.filter(item => !item.adminOnly || isPlatformAdmin).map((item) => {
