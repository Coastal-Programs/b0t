...
import { getOAuthCallbackConfig } from '@/lib/oauth-callback-urls';
...
export function CredentialCard({ credential, onDeleted, onUpdated }: CredentialCardProps) {
  ...
  const oauthConfig = getOAuthCallbackConfig(credential.platform);

  // OAuth connect handler
  const handleOAuthConnect = () => {
    // Extract platform from OAuth config (outlook_oauth_app -> outlook)
    const authPlatform = credential.platform.replace('_oauth_app', '');
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            {/* OAuth Connection Status - shown for OAuth app credentials */}
            {oauthConfig && (
              <div className="space-y-3 p-4 bg-muted/50 rounded-lg border">
                {credential.connectedAccount ? (
                  <>
                    ...
                    <div className="flex items-center gap-2">
                      <Label className="text-sm">Reconnect:</Label>
                      <Button
                        type="button"
                        onClick={handleOAuthConnect}
                        size="sm"
                        className="bg-white dark:bg-white hover:bg-black dark:hover:bg-black text-black dark:text-black hover:text-white dark:hover:text-white border border-gray-300 hover:border-black shadow-sm hover:shadow-md transition-all font-medium"
                        variant="outline"
                      >
                        {credential.platform === 'outlook_oauth_app' && (
                          <svg className="h-4 w-4 mr-2" viewBox="0 0 23 23" fill="none">
                            <rect width="23" height="23" rx="4" fill="url(#microsoft-gradient-edit)"/>
                            <path d="M1 1h10v10H1z" fill="#f25022"/>
                            <path d="M12 1h10v10H12z" fill="#00a4ef"/>
                            <path d="M1 12h10v10H1z" fill="#7fba00"/>
                            <path d="M12 12h10v10H12z" fill="#ffb900"/>
                            <defs>
                              <linearGradient id="microsoft-gradient-edit" x1="0" y1="0" x2="23" y2="23">
                        ...
                        {credential.platform === 'google_oauth_app' && (
                        ...
                        {credential.platform === 'youtube_oauth_app' && (
                          <svg className="h-4 w-4 mr-2" viewBox="0 0 24 24" fill="#FF0000">
                            <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                          </svg>
                        )}
                        {credential.platform === 'twitter_oauth_app' && (
                          <svg className="h-4 w-4 mr-2" viewBox="0 0 24 24" fill="#1DA1F2">
                            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                          </svg>
                        )}
                        Sign in with {credential.platform === 'outlook_oauth_app' ? 'Microsoft' : credential.platform === 'google_oauth_app' ? 'Google' : credential.platform === 'youtube_oauth_app' ? 'YouTube' : 'Twitter'}
                      </Button>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Connect your {credential.platform === 'outlook_oauth_app' ? 'Microsoft' : credential.platform === 'google_oauth_app' ? 'Google' : credential.platform === 'youtube_oauth_app' ? 'YouTube' : 'Twitter'} account
                        </p>
                      </div>
                    </div>
                    <Button
                    ...
                    >
                      {credential.platform === 'outlook_oauth_app' && (
                        <svg className="h-5 w-5 mr-2" viewBox="0 0 23 23" fill="none">
                          <rect width="23" height="23" rx="4" fill="url(#microsoft-gradient-edit2)"/>
                          <path d="M1 1h10v10H1z" fill="#f25022"/>
                          <path d="M12 1h10v10H12z" fill="#00a4ef"/>
                          <path d="M1 12h10v10H1z" fill="#7fba00"/>
                          <path d="M12 12h10v10H12z" fill="#ffb900"/>
                          <defs>
                            <linearGradient id="microsoft-gradient-edit2" x1="0" y1="0" x2="23" y2="23">
                              <stop offset="0%" stopColor="#f25022"/>
                              <stop offset="100%" stopColor="#ffb900"/>
                            </linearGradient>
                          </defs>
