...
export function MermaidDiagram({ chart, className }: MermaidDiagramProps) {
      ...
      if (ref.current && renderedChartRef.current !== chart && mermaidInstance) {
        try {
          ...
          if (ref.current) {
            ...
            codeEl.className = 'text-sm text-muted-foreground';
            codeEl.textContent = chart;
            ref.current.replaceChildren(codeEl);
            renderedChartRef.current = chart;
          }
        }
      }
  ...
  if (isLoading) {
    return (
      <div
        className={`my-3 p-4 border border-border rounded-lg bg-black/20 overflow-x-auto flex items-center justify-center min-h-[100px] ${className ?? ''}`}
      >
  ...
  return (
    <div
      ref={ref}
      className={`my-3 p-4 border border-border rounded-lg bg-black/20 overflow-x-auto min-h-[100px] ${className ?? ''}`}
    />
  );
}
