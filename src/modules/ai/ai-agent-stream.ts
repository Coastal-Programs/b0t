...
export interface StreamAgentOptions {
  ...
  /**
   * Tool selection options (shared with non-streaming agent via AgentToolOptions)
   */
...
export async function* streamAgent(
  ...
  // Resolve tools via shared resolver (same logic as non-streaming agent)
  const { tools: allTools, counts: toolCounts } = await resolveAgentTools(toolOptions, customTools);

  logger.info(toolCounts, 'Tools loaded for streaming agent');
          ...
          const step: ToolCallStep = {
            ...
            input:
              'input' in toolCall
                ? toolCall.input
                : 'args' in toolCall
                  ? (toolCall as unknown as { args: unknown }).args
                  : undefined,
            timestamp: new Date(),
          };
          allSteps.push(step);
 ...
 */
export async function runStreamingAgent(options: StreamAgentOptions): Promise<{
  text: string;
  steps: AgentStep[];
  usage?: { promptTokens: number; completionTokens: number; totalTokens: number };
}> {
