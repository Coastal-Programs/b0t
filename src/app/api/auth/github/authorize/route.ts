...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = searchParams.get('service') || 'github_oauth_service';
    ...
    if (platformCreds) {
      ...
      logger.info({ userId: session.user.id }, 'Using platform-wide GitHub OAuth credentials');
    }
      ...
      if (clientIdSetting) {
        try {
          ...
          logger.info(
            ...
            'Using Platform Settings GitHub OAuth credentials'
          );
        } catch (e) {
    ...
    if (!clientId) {
      ...
      if (!appCred) {
        ...
        return NextResponse.json(
          {
            error:
              'GitHub OAuth app not configured. Please contact admin or add your own GitHub OAuth App Credentials in the credentials page.',
          },
          { status: 500 }
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: error instanceof Error ? error.message : 'Invalid GitHub OAuth app credentials',
          },
          { status: 500 }
        );
      }
    ...
    const callbackUrl = process.env.NEXTAUTH_URL
      ...
      : 'http://localhost:3123/api/auth/github/callback';
    ...
    logger.info(
      { userId: session.user.id, provider: 'github', service },
      'Generated GitHub OAuth authorization URL'
    );

    return NextResponse.redirect(authUrl.toString());
  } catch (error) {
    logger.error({ error }, 'Failed to generate GitHub OAuth URL');
    return NextResponse.json({ error: 'Failed to initiate GitHub authorization' }, { status: 500 });
  }
}
